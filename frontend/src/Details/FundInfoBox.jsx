...
const FundInfoBox = ({ fund }) => {
  ...
  return (
    <Card bordered={false} style={{ borderRadius: 16, boxShadow: "0 4px 10px rgba(0,0,0,0.08)", padding: 5 }}>
      <div style={{ display: "flex", alignItems: "center", marginBottom: 16 }}>
        <img
          src={fund.creatorLogo || "https://upload.wikimedia.org/wikipedia/commons/thumb/7/79/Charity_logo.svg/512px-Charity_logo.svg.png"}
          alt={fund.creatorName}
          width={60}
          height={60}
          style={{ marginRight: 12 }}
        />
        <div>
          <Text strong style={{ fontSize: 20 }}>{fund.creatorName}</Text>
          <br />
          {fund.saoKeLink && <a href={fund.saoKeLink} style={{ fontSize: 16, color: "#52c41a" }}>Xem sao kê tài khoản</a>}
        </div>
      </div>
